...
![Linear discriminant analysis (LDA) of ecological network metrics for the seven model types. Each point represents a replicate, and ellipses indicate 95% confidence regions for each model. The second column represents the correlation of the various network metrics with the respective LDA axes. Left is before putting into END and right is post simulation structure (equilibrium topology)](figures/lda_compare.png){#fig-structure}
